...
export const renderGameState = (room_info: Room, username: string, sendJsonMessage: (em: EventMessage) => void) => {
    ...
    const current_users_seat_index = room_info.public_game_state.seats.findIndex(u => u?.username === username)
      ...
      {room_info.public_game_state.seats.map((player, index) => {
        if(!player){
          ...
          <>
            ...
            {
              current_users_seat_index === -1 &&  <button onClick={() => sendJsonMessage({uuid, messageType: MessageType.JOIN_GAME, actionAmount: index+1} as EventMessage)}>Join Game at seat {index+1}</button>
            }
          </>
          )
        }
        return (
        <>
          <p key={player?.id}>Seat {index + 1}: {player.username} </p>
          {
            current_users_seat_index === index && <button onClick={() => sendJsonMessage({uuid, messageType: MessageType.LEAVE_GAME} as EventMessage)}>Leave Game</button>
          }
        </>
        )
      })}
      <button onClick={() => sendJsonMessage({messageType: MessageType.START_GAME})} disabled={player_count<2}>Start Game</button>
       <h2>Table cards</h2>
